...
const accountStorage2: AccountStorage = {
  cashBalance: new BigNumber('-1300.23'), // 1000 - 2300.23
  positionAmount: new BigNumber('2.3'),
  entryValue: new BigNumber('2300.23'),
  entryFundingLoss: new BigNumber('-0.91'),
}
...
const accountDetails1 = computeAccount(perpetualStorage, accountStorage1)
const accountDetails3 = computeAccount(perpetualStorage, accountStorage3)
const accountDetails4 = computeAccount(perpetualStorage, accountStorage4)

// fundingLoss: 23.69365625, // 9.9059375 * 2.3 -(-0.91)
// availableCashBalance = 10000 - 2300.23 - 23.69365625 = 7676.07634375
const ammStorage1: AccountStorage = { ...accountStorage1 }

// fundingLoss: -23.69365625, // 9.9059375 * (-2.3) -(-0.91)
// availableCashBalance = 14000 + 2300.23 + 23.69365625 = 16323.92365625
// unsafe
const ammStorage3: AccountStorage = { ...accountStorage3 }
...
describe('computeAccount', function () {
  ...
  successCases.forEach(element => {
    it(element.name, function () {
      ...
      const accountDetails = computeAccount(perpetualStorage, accountStorage)
...
describe('computeTrade fail', function () {
  it('decrease flat', function () {
    expect((): void => {
      computeDecreasePosition(perpetualStorage, accountStorage4, new BigNumber(7000), _1)
    }).toThrow()
  })

  it('decrease zero price', function () {
    expect((): void => {
      computeDecreasePosition(perpetualStorage, accountStorage1, _0, _1)
    }).toThrow()
  })

  it('decrease zero amount', function () {
    expect((): void => {
      computeDecreasePosition(perpetualStorage, accountStorage1, _1, _0)
    }).toThrow()
  })

  it('decrease large amount', function () {
    expect((): void => {
      computeDecreasePosition(perpetualStorage, accountStorage1, _1, new BigNumber(1000))
    }).toThrow()
  })
  ...
  it('increase zero price', function () {
    expect((): void => {
      computeIncreasePosition(perpetualStorage, accountStorage1, _0, _1)
    }).toThrow()
  })

  it('increase zero amount', function () {
    expect((): void => {
      computeIncreasePosition(perpetualStorage, accountStorage1, _1, _0)
    }).toThrow()
  })
  ...
  it('computeTradeWithPrice zero price', function () {
    expect((): void => {
      computeTradeWithPrice(perpetualStorage, accountStorage1, _0, _1, _0)
    }).toThrow()
  })

  it('computeTradeWithPrice zero amount', function () {
    expect((): void => {
      computeTradeWithPrice(perpetualStorage, accountStorage1, _1, _0, _0)
    }).toThrow()
  })
})
...
describe('computeTradeWithPrice', function () {
  ...
  tradeCases.forEach(element => {
    ...
    it(name, function () {
      ...
      const details = computeAccount(perpetualStorage, newAccount)
...
describe('computeAMMPrice', function () {
  it(`holds long, sell`, function () {
    const { tradingPrice } = computeAMMPrice(perpetualStorage, ammStorage1, '-0.5')
  ...
  it(`holds long, buy cross 0`, function () {
    const { tradingPrice } = computeAMMPrice(perpetualStorage, ammStorage1, '3.3')
  ...
  it(`holds short, sell cross 0`, function () {
    const { tradingPrice } = computeAMMPrice(perpetualStorage, ammStorage3, '-2.4')
  ...
  it(`buy too large`, function () {
    expect((): void => {
      computeAMMPrice(perpetualStorage, ammStorage1, 11) // > 2.3 + 8.4
    }).toThrow()
  })

  it(`sell too large`, function () {
    expect((): void => {
      computeAMMPrice(perpetualStorage, ammStorage1, -10.1) // 12.4 - 2.3
    }).toThrow()
  })
})
...
describe('computeAMMTrade', function () {
  it(`sell`, function () {
    const res = computeAMMTrade(perpetualStorage, accountStorage1, ammStorage1, '-0.5')
  ...
  it(`buy cross 0`, function () {
    const res = computeAMMTrade(perpetualStorage, accountStorage1, ammStorage1, '3.3')
